...
export function renderPopupHTML(resort: Resort): string {
  ...
  return `
    <div style="font-family: Inter, -apple-system, sans-serif; padding: 8px; min-width: 280px;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
        <strong style="font-size: 18px; color: #1e293b;">${resort.name}</strong>
        <span style="background: ${color}22; color: ${color}; padding: 3px 10px; border-radius: 12px; font-size: 13px; font-weight: 600;">
          ${label}
        </span>
      </div>
      ...
      ${resort.condition.freezeThawRisk ? '<div style="margin-top: 10px; font-size: 13px; color: #f59e0b;">⚠ Freeze-thaw risk — possible icy conditions</div>' : ""}
    </div>
  `;
}
